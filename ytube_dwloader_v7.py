...
Change Log (v7)
- Duplicated from v6 as a new versioned file.
- Prefer running yt-dlp from the local rwakiDev_v3 Python environment:
    `rwakiDev_v3\\Scripts\\python.exe -m yt_dlp`
- Fall back to PATH-based yt-dlp only when rwakiDev_v3 is not available.
- Updated app ID and window title to v7.

Previous Change Log (v6)
- Duplicated from v5 as a new versioned file.
...
- Switched button order so Paste appears before Download Video.
- Updated Paste behavior to replace entry text with the latest clipboard content.
- Renamed Start to Download Video.
- Added Download as .WAV button for audio-only WAV downloads.
- Updated main window title to "Youtube Donwloader V6.00".
- Expanded TODO roadmap for in-app logging, update checks/patching, silent execution, progress bar, installer packaging, and desktop shortcut creation.
- Built v6 executable artifact: `dist/Youtube Downloader V6.00.exe`.
...
- Buttons: Paste, Download Video, Download as .WAV, Exit
- Download Video opens a new PowerShell window and runs yt-dlp with cookies + args
- Download as .WAV opens a new PowerShell window and downloads audio-only as WAV
- Paste clears existing text and inserts the latest clipboard contents

TODO
- Add a small status/log window below the main Tkinter window to track warnings, errors, and out-of-date notices.
- Add an app version check that reports whether yt-dlp is up to date.
- Add a "Patch to Latest" button that appears or enables only when an update is available.
- Close the PowerShell window automatically when a download/update process is complete.
- Run PowerShell silently (no visible console) while streaming process updates into the main Tkinter log.
- Add a progress bar in the Tkinter UI and make progress reporting as accurate as possible.
- Package this as an installable Windows program (installer-based setup).
- Add desktop shortcut/icon creation during installation.
- Add a clear in-file section describing how the code works (high-level flow + key functions).
- Create README files for each build version with that build's changelog and a description of how the program works.
"""
...
APP_VERSION = "V7.00"
APP_ID = "Kwa.ytube_dwloader.v7"
VENV_DIR_NAME = "rwakiDev_v3"
...
def find_rwaki_v3_python() -> Path | None:
    python_name = "python.exe" if sys.platform.startswith("win") else "python"
    candidates = [
        script_dir() / VENV_DIR_NAME / "Scripts" / python_name,
        script_dir() / VENV_DIR_NAME / "bin" / python_name,
    ]
...
def get_ytdlp_command() -> list[str] | None:
    venv_python = find_rwaki_v3_python()
    if venv_python:
...
def main():
    ...
    def start_with_entry(download_wav_only: bool = False):
        ...
        try:
            ...
            if not ytdlp_command:
                messagebox.showerror(
                    "Not found",
                    f"yt-dlp was not found in {VENV_DIR_NAME} or on PATH.\n"
                    f"Install it with: {VENV_DIR_NAME}\\Scripts\\python.exe -m pip install yt-dlp",
                )
                return
            ...
            parts = ytdlp_command + js_args + ["--cookies", r"C:\Users\kirwa\Documents\yt-dlp\ytube_cookies.txt", "-P", download_dir]

            if download_wav_only:
                # Force audio extraction to WAV when using the WAV button.
                parts.extend([
                    "-f",
                    "bestaudio/best",
                    "-x",
                ...
                if user_text:
                    ...
                    parts.extend(default_video_args)
    ...
    run_btn = tk.Button(btn_frame, text="Download Video", width=14, command=start_with_entry)
    run_btn.pack(side=tk.LEFT, padx=6)

    wav_btn = tk.Button(btn_frame, text="Download as .WAV", width=16, command=lambda: start_with_entry(True))
    wav_btn.pack(side=tk.LEFT, padx=6)
