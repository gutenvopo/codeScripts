"""
Speed Test GUI Application v1.03
================================================================================
...
class SpeedTestApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Speed Test Application v1.03")
        ...
        try:
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if '403' in str(e):
                        retry_count += 1
                        self.log_output(f"\n[ERROR] 403 Forbidden Error encountered")
            ...
            try:
                ...
                if '403' in str(e):
                    self.log_output(f"\n[ERROR] 403 Forbidden during download from server: {best_server.get('sponsor', 'Unknown')}")
                    self.log_output(f"Location: {best_server.get('name', 'N/A')}, {best_server.get('country', 'N/A')}")
                    self.log_output("Attempting to select alternate server...")
                    st.get_servers()
                    best_server = st.get_best_server()
                    self.log_output(f"Retrying with alternate server: {best_server.get('sponsor', 'Unknown')}")
                    st.download()
            ...
            try:
                ...
                if '403' in str(e):
                    self.log_output(f"\n[ERROR] 403 Forbidden during upload from server: {best_server.get('sponsor', 'Unknown')}")
                    self.log_output(f"Location: {best_server.get('name', 'N/A')}, {best_server.get('country', 'N/A')}")
                    self.log_output("Attempting to select alternate server...")
                    st.get_servers()
                    best_server = st.get_best_server()
